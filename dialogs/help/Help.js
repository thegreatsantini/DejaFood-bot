 ...
 */
class Help extends ComponentDialog {
    constructor(dialogId, userProfileAccessor) {
        ...
        this.addDialog(new WaterfallDialog(HELP_DIALOG, [
        ...
        this.addDialog(new TextPrompt(HELP_PROMPT));
    ...
    async promptForNameStep(step) {
        ...
        if (!userProfile.name) {
            ...
            return await step.prompt(NAME_PROMPT, 'Ok, before we started let\'s create a profile, can you tell me your name?');
        } else {
            return await step.next();
        }
    }
    ...
    async promptForRecipeSearchStep(step) {
        ...
        // console.log('inside .set()',userProfile.name, step.result)
        ...
        // if there is a name 
        if (userProfile.name) {
            // console.log('userProfile.name is true',userProfile)
            return await step.prompt(SEARCH_PROMPT, `Ok ${userProfile.name}, You are now ready to search for recipes. 
                                                    You can do this by simply listing indgredients aor by saying a phrase like what can I cook with <Ingredrients>`);
        } else {
            return await step.next();
        }
    }
    ...
    async greetUser(step) {
        ...
        await step.context.sendActivity(`By the way. You can always say 'My name is <your name> to reintroduce yourself to me.`);
        return await step.endDialog();
    }
}

exports.HelpDialog = Help;